...
def startup():
    try:
        ...
        super_admin_role = db.query(models.Role).filter(models.Role.role_name == "SuperAdmin").first()
        if super_admin_role:
            existing_user = db.query(models.User).filter(models.User.email == "denisv@ost.com").first()
            ...
            existing_personal_detail = db.query(models.PersonalDetail).filter(models.PersonalDetail.user_id == user.id).first()
            if not existing_personal_detail:
                personal_detail = models.PersonalDetail(
                    user_id=user.id,
                    first_name='Den',
                    last_name='V',
        ...
        existing_company = db.query(models.Company).filter(models.Company.email == 'contact@techinnovators.com').first()
        ...
        existing_connect_company = db.query(models.ConnectCompany).filter(
            models.ConnectCompany.worker_id == user.id,
            models.ConnectCompany.company_id == company.id
        ).first()
        
        if not existing_connect_company:
            connect_company = models.ConnectCompany(
                worker_id=user.id,
...
async def create_connectCompany(connectCompany: ConnectCompanyBase, db: db_dependency):    
    db_connectCompany = models.ConnectCompany(**connectCompany.dict())
...
async def read_connectCompany(connectCompany_id:int, db: db_dependency):  
    connectCompany = db.query(models.ConnectCompany).filter(models.ConnectCompany.id == connectCompany_id).first
