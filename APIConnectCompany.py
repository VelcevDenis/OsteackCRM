...
@router.get("/recallCompanis/{connectCompany_id}", status_code=status.HTTP_200_OK)
...
@router.get("/recallCompanis/{worker_id}", status_code=status.HTTP_200_OK, response_model=List[column_models.ConnectCompanyBase])
...
# ??????
# @router.get("/recallCompanis/all", status_code=status.HTTP_200_OK, response_model=List[column_models.ConnectCompanyBase])
# async def read_all_connect_company(u: user_dependency, db: db_dependency) -> List[column_models.ConnectCompanyBase]:  
#     companis = db.query(columns.ConnectCompanis).all()    
#     return [column_models.CompanyBase.from_orm(company) for company in companis]
# ??????