...
async def create_user(db: db_dependency, create_user_request: CreateUserRequest):
    ...
    create_user_model = Users(
        ...
        hashed_pass = mAuth.bcrypt_context.hash(create_user_request.hashed_pass),         
...
async def login_for_access_token(from_data: Annotated[OAuth2PasswordRequestForm, Depends()],
                                  db: db_dependency):
    user = mAuth.authenticate_user(from_data.username, from_data.password, db)  # Use username here
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Could not validate user.",
        )
    token = mAuth.create_access_token(user, timedelta(minutes=SESSION_TIME))
    return {"access_token": token, "token_type": "bearer"}

@router.get("/verify-token/{token}")
async def auth_user_token_verification(token:str):
    info = await mAuth.get_current_user(token=token)
    return {"info":info}